...
class Graduate extends Model
{
  use HasFactory;
   ...
   protected $fillable = [
    ...
    //relacion uno a muchos porque un cuerpo de soldados puede tener muchos soldados pero un soldado solo puede pertenecer a un cuerpo de soldados
...
// relacion muchos a muchos porque un soldado puede tener muchos servicios y un servicio puede ser prestado por muchos soldados
    ...
    // los Scopes son funciones que se pueden llamar en la consulta de un modelo para modificar su comportamiento, por ejemplo: Soldier::included()->filter()->sort()->get() o Soldier::included()->filter()->sort()->paginate(10)
     ...
     public function scopeFilter(Builder $query)
    {
        ...
        foreach ($filters as $filter => $value) {
            ...
            // filtro por relación many-to-many
            if ($filter == 'title') {

                $query->whereHas(
                    'title',
                    function ($q) use ($value) {
                        $q->where('title.id', $value);
                    }
                );
    ...
    public function scopeSort(Builder $query)
    {
     ...
     empty(request('sort'))
      ...
      foreach ($sortFields as $sortField) {
        ...
        return $query;
        //http://soldados.test/soldiers?sort=nombre
        ///soldiers?sort=nombre
        ///soldiers?sort=-apellido
    ...
    public function scopeGetOrPaginate(Builder $query)
    {
      if (request('perPage')) {
        ...
        //http://soldados.test/soldiers?perPage=2
        ///soldiers?perPage=2
    }
}
